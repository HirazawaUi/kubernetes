...
func WriteInstanceConfigToDisk(nodeReg *kubeadmapi.NodeRegistrationOptions, kubeletDir string) error {
	kubeletCfg := &kubeletconfig.KubeletConfiguration{
		ContainerRuntimeEndpoint: nodeReg.CRISocket,
	}

	// writes the instance kubelet config to disk
	serialized, err := kubeadmutil.MarshalToYaml(kubeletCfg, kubeletconfig.SchemeGroupVersion)
	if err != nil {
		return errors.Wrapf(err, "failed to marshal manifest for instance kubelet config to YAML")
	}

	return writeInstanceConfigBytesToDisk(serialized, kubeletDir)
}
